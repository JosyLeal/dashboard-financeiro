...
const FILE = "Controle_gastos_Exemplo.xlsx";
...
function parseXlsx(excelPath) {
  ...
  for (let i = 2; i <= maxRow; i++) {
    ...
    records.push({
      ...
      centroCusto: normalizeText(get(r, "Centro de custo")),
      essencial: normalizeText(get(r, "Essencial")),
      observacoes: normalizeText(get(r, "Observações")),
    });
  }
...
if (!fs.existsSync(fullPath)) {
  ...
  console.error("Execute: node scripts/generate-sample-xlsx.js");
  process.exit(1);
}

const all = parseXlsx(fullPath);
console.log(`Exemplo: ${all.length} lançamentos`);

const meta = {
  titulo: "Dashboard Financeiro — Base de Estudo",
  fonte: FILE,
  objetivoMensal: 3500,
  objetivoCartao: 800,
  cor: "#5b8def",
...
fs.writeFileSync(path.join(BASE, "data-exemplo.json"), JSON.stringify(all), "utf8");

const dataJs = `window.FINANCE_DATA=${JSON.stringify(all)};\nwindow.FINANCE_META=${JSON.stringify(meta)};\n`;
fs.writeFileSync(path.join(BASE, "js", "data-exemplo.js"), dataJs, "utf8");
console.log("Exportado: js/data-exemplo.js e data-exemplo.json");
